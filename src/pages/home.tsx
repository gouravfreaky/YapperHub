...
// Define UserProfile type directly since we're removing the shared schema
...
export default function Home() {
  ...
  // Fetch user data directly from Kaito API when searchedUsername is set
  const { data: userData, isLoading, error, refetch } = useQuery<UserProfile>({
    queryKey: ["kaito-user", searchedUsername],
    queryFn: async () => {
      const response = await fetch(`https://api.kaito.ai/api/v1/yaps?username=${encodeURIComponent(searchedUsername)}`);
